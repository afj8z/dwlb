...
#define USAGE                                                                  \
  "usage: dwlb [OPTIONS]\n"                                                    \
  "Ipc\n"                                                                      \
  "	-ipc				allow commands to be sent to dwl "                                 \
  "(dwl must be patched)\n"                                                    \
  "	-no-ipc				disable ipc\n"                                                  \
  "Bar Config\n"                                                               \
  ...
  if (i < tags_l) {
    ...
    if (ipc) {
      if (seat->pointer_button == BTN_LEFT)
        ...
        zdwl_ipc_output_v2_set_tags(seat->bar->dwl_wm_output,
                                    seat->bar->mtags ^ (1 << i), 0);
    }
  } else if (seat->pointer_x <
             (x += TEXT_WIDTH(seat->bar->layout, seat->bar->width - x,
                              seat->bar->textpadding))) {
    /* Clicked on layout */
    if (ipc) {
      if (seat->pointer_button == BTN_LEFT)
        zdwl_ipc_output_v2_set_layout(seat->bar->dwl_wm_output,
                                      seat->bar->last_layout_idx);
      else if (seat->pointer_button == BTN_RIGHT)
        zdwl_ipc_output_v2_set_layout(seat->bar->dwl_wm_output, 2);
    }
  } else {
    uint32_t status_x = seat->bar->width / buffer_scale -
                        TEXT_WIDTH(seat->bar->status.text, seat->bar->width - x,
                                   seat->bar->textpadding) /
                            buffer_scale;
    if (seat->pointer_x < status_x) {
      ...
      if (custom_title) {
        if (center_title) {
          uint32_t title_width =
              TEXT_WIDTH(seat->bar->title.text, status_x - x, 0);
          x = MAX(x, MIN((seat->bar->width - title_width) / 2,
...
static const struct zdwl_ipc_manager_v2_listener dwl_wm_listener = {
    .tags = dwl_wm_tags, .layout = dwl_wm_layout};

static void
dwl_wm_output_toggle_visibility(void *data,
                                struct zdwl_ipc_output_v2 *dwl_wm_output) {
  Bar *bar = (Bar *)data;
...
static void dwl_wm_output_active(void *data,
                                 struct zdwl_ipc_output_v2 *dwl_wm_output,
                                 uint32_t active) {
  Bar *bar = (Bar *)data;
...
static void dwl_wm_output_tag(void *data,
                              struct zdwl_ipc_output_v2 *dwl_wm_output,
...
static void dwl_wm_output_frame(void *data,
                                struct zdwl_ipc_output_v2 *dwl_wm_output) {
  Bar *bar = (Bar *)data;
  bar->redraw = true;
}

static void dwl_wm_output_fullscreen(void *data,
                                     struct zdwl_ipc_output_v2 *dwl_wm_output,
                                     uint32_t is_fullscreen) {}

static void dwl_wm_output_floating(void *data,
                                   struct zdwl_ipc_output_v2 *dwl_wm_output,
...
static void setup_bar(Bar *bar) {
  ...
  if (ipc) {
    ...
    zdwl_ipc_output_v2_add_listener(bar->dwl_wm_output, &dwl_wm_output_listener,
                                    bar);
  }
...
int main(int argc, char **argv) {
  ...
  for (uint32_t i = 0; i < 50; i++) {
    if ((sock_fd = socket(AF_UNIX, SOCK_STREAM, 1)) == -1)
      DIE("socket");
    snprintf(sock_address.sun_path, sizeof sock_address.sun_path, "%s/dwlb-%i",
             socketdir, i);
    if (connect(sock_fd, (struct sockaddr *)&sock_address,
                sizeof sock_address) == -1) {
      found = true;
